...
if ($bookmark_url && $bookmark_title) {
    if (true === bookmark_add($bookmark_url, $bookmark_title)) {
        redirect_header(XOOPS_URL . '/modules/xfaccount/', 4, sprintf(_XF_MY_ADDEDBOOKMARK, $bookmark_url, $bookmark_title));
    } else {
        redirect_header(Request::getString('HTTP_REFERER', '', 'SERVER'), 4, 'Could not add bookmark');
    }
} else {
    include '../../header.php';
    ...
    <form method="post">
        ...
        <input type="text" name="bookmark_url" value="<?php echo $bookmark_url ?: 'http://'; ?>">
        <p>
            <?php echo _XF_MY_BOOKMARKTITLE; ?>:<br>
            <input type="text" name="bookmark_title" value="<?php echo $bookmark_title ?: 'New Title'; ?>">
        <p>
