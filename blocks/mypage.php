    ...
    if (!$result || $rows < 1) {
        ...
        while (false !== ($row = $xoopsDB->fetchArray($result))) {
            if ($row['group_id'] != $last_group) {
                $content .= "<TR><TD><B><A HREF='" . $XOOPS_URL . '/modules/xfmod/project/?' . $row['unix_group_name'] . "'>" . $ts->htmlSpecialChars($row['group_name']) . '</A></TD></TR>';
            }

            $content .= "<TR><TD id=''><A HREF='" . $XOOPS_URL . '/modules/xfmod/project/showfiles.php?group_id=' . $row['group_id'] . "'>" . $ts->htmlSpecialChars($row['name']) . '</A></TD></TR>';
