...
$GLOBALS['xoopsOption']['template_main'] = 'xfaccount_pubkeys.html';
...
function getLDAPConnection()
{
    ...
    if (!$lldap->connect()) {
        $xoopsForgeErrorHandler->setSystemError(
            'Failed to connect to LDAP server: ' . $xoopsConfig['ldapserver']
        );
    }
    ...
    if (!$lldap->bindAdmin()) {
        ...
        $xoopsForgeErrorHandler->setSystemError(
            'Failed to bind to LDAP server: ' . $ldaperr
        );
    }
...
if ($func) {
    ...
    if ('delete' == $func) {
        if (mb_strlen($pkey) && $pkeytime > 0) {
            $res = $xoopsDB->queryF(
                'DELETE FROM ' . $xoopsDB->prefix('xf_pubkeys') . " WHERE uid='" . $xoopsUser->getVar('uid') . "' AND time='" . $pkeytime . "'"
            );

            if (!$res) {
                $xoopsForgeErrorHandler->addError(
                    'Failed to remove public key. ' . 'Error: ' . $xoopsDB->error()
                );
            } else {
                $doLDAPTrans = true;
    ...
    } elseif ('deleteall' == $func) {
        $res = $xoopsDB->queryF(
            'DELETE FROM ' . $xoopsDB->prefix('xf_pubkeys') . " WHERE uid='" . $xoopsUser->getVar('uid') . "'"
        );

        if (!$res) {
            $xoopsForgeErrorHandler->addError(
                'Failed to remove all public keys. ' . 'Error: ' . $xoopsDB->error()
            );
        } else {
            $doLDAPTrans = true;
        ...
        if (mb_strlen($pkey) > 0) {
            $sql = 'INSERT INTO ' . $xoopsDB->prefix('xf_pubkeys') . ' (uid, time, pubkey) values (' . $xoopsUser->getVar('uid') . ',' . time() . ',' . "'" . $pkey . "')";

            $res = $xoopsDB->queryF($sql);

            if (!$res) {
                $xoopsForgeErrorHandler->addError(
                    'Failed to add public key. ' . 'Error: ' . $xoopsDB->error() . ' SQL: ' . $sql
                );
            } else {
                $doLDAPTrans = true;
    ...
    if ($doLDAPTrans) {
        ...
        $res = $xoopsDB->query(
            'SELECT pubkey FROM ' . $xoopsDB->prefix('xf_pubkeys') . " WHERE uid='" . $xoopsUser->getVar('uid') . "'"
        );

        if ($res && $xoopsDB->getRowsNum($res) > 0) {
            while (false !== ($therow = $xoopsDB->fetchArray($res))) {
        ...
        if (!$rc) {
            $xoopsForgeErrorHandler->addError(
                'Failed to add Public Key transaction. ' . 'Error: ' . $lldap->lastError()
            );
        } else {
...
$res = $xoopsDB->query(
    'SELECT time,pubkey FROM ' . $xoopsDB->prefix('xf_pubkeys') . " WHERE uid='" . $xoopsUser->getVar('uid') . "'"
);
...
if (!$res || $xoopsDB->getRowsNum($res) < 1) {
    ...
    while (false !== ($therow = $xoopsDB->fetchArray($res))) {
        ...
        $fpkey = chunk_preg_split($pubkey, 80, '<br>');

        $cdate = date($sys_datefmt, $keytime);

        $content .= '<tr class="' . (0 != $i % 2 ? 'odd' : 'even') . '"><td>' . "$cdate</td><td><br>$fpkey<br></td>";

        $content .= "<td align='center'><form action='"
                    . $_SERVER['PHP_SELF']
                    . "' method='post'"
                    ...
                    . $pubkey
                    . "'>"
                    . "<input type='hidden' name='pkeytime' value='"
                    . $keytime
                    . "'>"
    ...
    $content .= "<tr><td colspan='3'>&nbsp;&nbsp;</td></tr>"
                . "<tr><td align='center' colspan='3'><form action'"
                . $_SERVER['PHP_SELF']
                . "' method='post'"
