...
$GLOBALS['xoopsOption']['template_main'] = 'xfaccount_diary.html';
...
if ($xoopsUser) {
    ...
    if ($submit) {
        ...
        if ($update) {
            ...
            $res = $xoopsDB->queryF(
                'UPDATE ' . $xoopsDB->prefix('xf_user_diary') . ' SET ' . "summary='" . $ts->addSlashes($summary) . "'," . "details='" . $ts->addSlashes($details) . "'," . "is_public='$is_public' " . "WHERE user_id='" . $xoopsUser->getVar('uid') . "' " . "AND id='$diary_id'"
            );
            ...
            $sql = 'INSERT INTO '
                   ...
                   . "',"
                   . "'"
                   . $ts->addSlashes($summary)
                   . "',"
                   . "'"
                   . $ts->addSlashes($details)
                   . "',"
                   . "'$is_public')";
            ...
            if ($res) {
                ...
                if ($is_public) {
                    ...
                    $sql = 'SELECT u.email ' . 'FROM ' . $xoopsDB->prefix('xf_user_diary_monitor') . ' udm,' . $xoopsDB->prefix('users') . ' u ' . 'WHERE udm.user_id=u.uid ' . 'AND u.level <> 0 ' . "AND udm.monitored_user='" . $xoopsUser->getVar('uid') . "'";
                    ...
                    if ($result && $rows > 0) {
                        ...
                        $message = myGetDiaryMessage($xoopsUser->getVar('uname'), $ts->htmlSpecialChars($summary), $ts->htmlSpecialChars($details), $xoopsUser->getVar('uid'));

                        xoopsForgeMail($xoopsForge['noreply'], $xoopsConfig['sitename'], $message['subject'], $message['body'], [$xoopsForge['noreply']], $bcc_arr);
    ...
    if ($diary_id) {
        $sql = 'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_user_diary') . ' ' . "WHERE user_id='" . $xoopsUser->getVar('uid') . "' " . "AND id='$diary_id'";
    ...
    $xoopsTpl->assign('summary', $ts->htmlSpecialChars($_summary));

    $xoopsTpl->assign('details', $ts->htmlSpecialChars($_details));
    ...
    $sql = 'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_user_diary') . ' ' . "WHERE user_id='" . $xoopsUser->getVar('uid') . "' " . 'ORDER BY id DESC';
    ...
    if (!$result || $rows < 1) {
        ...
        for ($i = 0; $i < $rows; $i++) {
            ...
            $diary_list[$i]['summary'] = $ts->htmlSpecialChars(unofficial_getDBResult($result, $i, 'summary'));
